...
async def get_cart(payload: dict, token: Annotated[str | None, Cookie()] ):
    ...
    # #     raise HTTPException(status_code=401, detail="Not logged in")
        username = jwt.decode(token, SECRET_KEY, algorithms=ALGORITHM)["email"]
        print("cart for user: " + username)
